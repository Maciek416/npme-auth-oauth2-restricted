...
AuthorizerOAuth2.prototype.matchesMasks = function (email) {
  return !!this.userWhitelist.find((whitelisted) => {
    return (whitelisted.indexOf("*") === 0 && email.split("@")[1]===whitelisted.split("@")[1])
  })
}
...
AuthorizerOAuth2.prototype.authorize = function (credentials, cb, skipWhitelist) {
  ...
  this.session.get(token, function (err, user) {
    ...
    else if (!skipWhitelist && _this.userWhitelist.indexOf(user.email) === -1 && !_this.matchesMasks(user.email)) {
...
function error402 (email) {
  var error = Error(email + ' is not authorized to use npm Enterprise, please contact an admin')
